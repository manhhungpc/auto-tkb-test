...
export async function getHTMLContent(filePath: string) {
	const htmlFile = readFileSync(filePath).toString('utf-8');
	const doc = new JSDOM(htmlFile, { contentType: 'text/html' });
	...
	writeFileSync('static/DSDK/subject_formated.json', JSON.stringify(dsdkData));
	return dsdkData;
}
