...
export async function exportExcel(scheduleData: Subjects) {
	...
	sheet.columns = [
		...
		{ header: 'Thứ 2', key: 'mon', width: 32 },
		{ header: 'Thứ 3', key: 'tue', width: 32 },
		{ header: 'Thứ 4', key: 'wed', width: 32 },
		{ header: 'Thứ 5', key: 'thu', width: 32 },
		{ header: 'Thứ 6', key: 'fri', width: 32 },
		{ header: 'Thứ 7', key: 'sat', width: 32 }
	];
	...
	for (let i = 0; i < scheduleData.subjects.length; i++) {
		...
		console.log([subject.name, rowStart.number, column, rowEnd.number, column]);
	}
